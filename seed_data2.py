...
3. exec(open('seed_data2.py').read())
...
from products.models import Category, Tag, Brand, Product, ProductImage, Review, StockStatus
from users.models import User

# Configure Cloudinary (already done in settings, but ensure it's loaded)
cloudinary.config(
...
# Corrected and adjusted product data from your JSON (20 items)
# Changes:
# - Removed id, sku (auto-generated).
# - Computed discount_percentage if current_price < original_price, else None.
# - Mapped additional_info to model fields (weight, color, type, stock_count).
# - Tags as list (will create Tag objects).
# - Category as string (will map to hierarchical Category).
# - Images as list of URLs (first primary).
# - Ignored isSale (mismatched, perhaps typo), rating/reviews_count/stock_status (computed via signals/stock_count).
# - Fixed mismatches (e.g., description tweaks, image_2 ignored or appended if present).
# - For images: Some are repeated; script will upload uniquely per product.
# - Stock_count >0 -> IN_STOCK automatically.
...
# Brands (added more, 10 total, no images for simplicity)
brands_list = [
...
# Users (8, with simple data)
users_data = [
    {"email": "test1@example.com", "password": "secuoire19u23"},
    {"email": "test2@example.com", "password": "tuek1sdf23"},
    {"email": "test3@example.com", "password": "secuoire19u23"},
    {"email": "test4@example.com", "password": "tuek1sdf23"},
    {"email": "test5@example.com", "password": "secuoire19u23"},
    {"email": "test6@example.com", "password": "tuek1sdf23"},
    {"email": "test7@example.com", "password": "secuoire19u23"},
    {"email": "test8@example.com", "password": "tuek1sdf23"}
]

# Sample comments for reviews
review_comments = [
    "Great product, very fresh!", "Loved the taste!", "Good value for money.",
    "Will buy again.", "Highly recommended.", "A bit pricey but worth it.",
    "Perfect for my recipes.", "Fresh and organic as advertised."
]
...
def seed_data():
    ...
    # Create categories hierarchically – more defensive version
    # ================== Category creation – full hierarchical & safe ==================
    ...
    # 2. Second level under "Fresh Produce"
    ...
    # 3. Third level (sub-sub categories)
    third_level = [
        ("Apples", "Fruits"),
        ("Mangoes", "Fruits"),
        ("Citrus", "Fruits"),
    ...
    for name, parent_key in third_level:
        if parent_key not in category_map:
            print(f"Warning: Parent {parent_key} not found for {name}")
            continue

        parent = category_map[parent_key]
    ...
    for data in all_products_data:
        # Map category (use subcategories where possible)
        cat_name = data["category"]  # "Fruits" or "Vegetables"

        if cat_name == "Fruits":
            # Try to be more specific
            if "Apple" in data["name"]:
                preferred = "Apples"
            elif "Mango" in data["name"]:
                preferred = "Mangoes"
            elif "Orange" in data["name"] or "Pineapple" in data["name"]:
                preferred = "Citrus"
            else:
                preferred = "Fruits"
            ...
            if any(word in data["name"] for word in ["Lettuce", "Spinach", "Cabbage", "Chinese Cabbage"]):
                preferred = "Leafy Greens"
            elif any(word in data["name"] for word in ["Carrot", "Potato", "Onion", "Beetroot", "Radish", "Ginger"]):
                preferred = "Root Vegetables"
            elif any(word in data["name"] for word in ["Pepper", "Capsicum", "Chilli", "Bell"]):
                preferred = "Peppers"
            elif any(word in data["name"] for word in ["Zucchini", "Pumpkin", "Squash"]):
                preferred = "Squash"
            elif "Cabbage" in data["name"]:
                preferred = "Cabbage"
            else:
        ...
        product = Product(
            ...
            discount_percentage=Decimal(data["discount_percentage"]) if data["discount_percentage"] else None,
            brand=brand,
            category=category,
        ...
        product.save()  # Triggers SKU auto-gen, current_price computation

        # Add tags
        for tag_name in data["tags"]:
            product.tags.add(tags_map[tag_name])

        # Upload and add images (only for first 20)
        if "images" in data and data["images"]:
            for idx, url in enumerate(data["images"]):
                try:
                    # Upload to Cloudinary (remote URL)
                    ...
                    ProductImage.objects.create(
                        product=product,
                        image=public_id,  # CloudinaryField stores public_id
                        alt_text=alt_text,
                        is_primary=is_primary
                    )
                except Exception as e:
                    print(f"Failed to upload image {url} for {data['name']}: {e}")

        products.append(product)

    # Create reviews
    # ────────────────────────────────────────────────
    # Reviews – safe against existing unique constraint
    # ────────────────────────────────────────────────
    ...
    review_comments = [
        ...
        "Love the quality - will order again.",
        "Good value, very satisfied.",
        "Perfect for salads and cooking.",
        "Organic and delicious.",
        "Highly recommend to everyone.",
        "A bit small but very flavorful.",
        "Exactly what I was looking for.",
        "Great taste, arrived in good condition.",
        "Best I've had in a while!",
    ]

    for product in products:
        # Try to create 4–8 reviews per product
        num_attempts = random.randint(5, 12)

        # Shuffle users to distribute reviews more evenly
        user_pool = list(users)
        random.shuffle(user_pool)

        for i in range(num_attempts):
            user = user_pool[i % len(user_pool)]   # cycle through users

            try:
                Review.objects.create(
                    product=product,
                    user=user,
                    rating=random.choices([3,4,4,5,5,5], k=1)[0],  # bias towards 4–5
                    comment=random.choice(review_comments)
                )
                created_reviews += 1
            except IntegrityError as exc:
                if "UNIQUE" in str(exc) and "review" in str(exc).lower():
                    skipped_reviews += 1
                    duplicate_attempts += 1
                else:
                    print(f"Unexpected error for {product.name} / {user.email}: {exc}")
                    raise

    print(f"→ Created {created_reviews} reviews")
    print(f"→ Skipped {skipped_reviews} duplicate attempts")
    if duplicate_attempts > 0:
        print("  (database still has unique constraint on (product, user))")
        # Signals will update average_rating and reviews_count
